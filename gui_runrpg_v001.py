...
#
program_name="gui_runrpg__v001.py"
#
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
def on_select(event):
    ...
    if selected_indices:
        # For single selection, take the first index
        selected_item_index = selected_indices[0]
        selected_value = my_listbox.get(selected_item_index)
        print(f"Selected item: {selected_value}")
        selected_label.config(text=f"You chose: {selected_value}")
        return(selected_value)
#
def show_message_and_exit1():
    #-Displays an information message box and then destroys the main window.
    messagebox.showinfo("Select the directory to store the ksh files.", "Select the directory to store the ksh files. Click OK to exit.")
    root.destroy()
 
def show_message_and_exit2():
    #-Displays an information message box and then destroys the main window.
    messagebox.showinfo("Select the Excel file to process.", "Select the Excel file to process. Click OK to exit.")
    root.destroy()

def show_message_and_exit3():
    #-Displays an information message box and then destroys the main window.
    messagebox.showinfo("Select the Excel tab or sheet to process.", "Select the Excel tab or sheet to process. Click OK to exit.")
    root.destroy()
...
ksh_scripts='/import/frb_archive/pmccrone/Scripts/runrpg/ksh_scripts2025A/' #### Make sure ksh_scripts ends with a /.
...
try:
    excel_file = PD.ExcelFile(file) # Replace 'path/to/your/excel_file.xlsx' with the actual path to your Excel file.
except:
    #
    print("---CAUTION---")
